...
public class HookMain implements IXposedHookLoadPackage {
    ...
    public static FlvVideoSource c2_flv_source;
    public static FlvVideoSource c2_flv_preview;
    public static LocalRotatePlayer c2_local_player;
    public static CameraCompositor compositor;
...
/*        XposedHelpers.findAndHookMethod("android.hardware.Camera", lpparam.classLoader, "stopPreview", new XC_MethodHook() {
    ...
    private void process_camera2_play() {
        ...
        if (isComposite() && compositor != null) {
            Surface out = (c2_preview_Surfcae != null) ? c2_preview_Surfcae : c2_reader_Surfcae;
            if (out != null) {
                float[] rect = readOverlayRect();
                File ovVideo = new File(video_path + "overlay.mp4");
        ...
        if (c2_reader_Surfcae != null) {
            if (c2_flv_source != null) {
                c2_flv_source.stopDecode();
                c2_flv_source = null;
            }
            String liveUrl = readLiveUrl();
            if (liveUrl != null) {
                c2_flv_source = new FlvVideoSource();
                c2_flv_source.setSurface(c2_reader_Surfcae);
                c2_flv_source.setRotation(readLiveRot());
                c2_flv_source.setAudioEnabled(readNoSilent());
                c2_flv_source.start(liveUrl);
                XposedBridge.log("【VCAM】【flv】Camera2 采集改用直播流：" + liveUrl);
            } else {
        ...
        if (c2_preview_Surfcae != null) {
            String liveUrlP = readLiveUrl();
            if (liveUrlP != null) {
                if (c2_flv_preview != null) {
                    c2_flv_preview.stopDecode();
                    c2_flv_preview = null;
                }
                c2_flv_preview = new FlvVideoSource();
                c2_flv_preview.setSurface(c2_preview_Surfcae);
                c2_flv_preview.setRotation(readLiveRot());
                c2_flv_preview.setAudioEnabled(readNoSilent());
                c2_flv_preview.start(liveUrlP);
                XposedBridge.log("【VCAM】【flv】Camera2 预览通道改用直播流：" + liveUrlP);
            } else {
                if (c2_local_player != null) { c2_local_player.stop(); c2_local_player = null; }
                int localRot = readLiveRot();
                if (localRot != 0) {
                    // 本地视频实时旋转（不预转码）：MediaPlayer 解码/循环/声音 + GL 旋转喂帧
                    c2_local_player = new LocalRotatePlayer();
                    c2_local_player.start(video_path + "virtual.mp4", c2_preview_Surfcae, localRot, readNoSilent());
                    XposedBridge.log("【VCAM】【local】预览通道实时旋转 " + localRot + "°");
                } else {
                    if (c2_player == null) {
    ...
    /** 读目标 App 私有目录里的 live_url.txt（http/https 开头才算有效），决定是否走直播流。 */
    private String readLiveUrl() {
        try {
            File f = new File(video_path + "live_url.txt");
    ...
    /** 读 live_rot.txt（0/90/180/270），直播流旋转角度。 */
    private int readLiveRot() {
        try {
            File f = new File(video_path + "live_rot.txt");
    ...
    /** “播放视频原声”开关：no-silent.jpg 存在则出声。 */
    private boolean readNoSilent() {
    ...
    private int readOverlayAngle() {
        try {
            File f = new File(video_path + "overlay_rot.txt");
            if (f.exists()) {
                java.io.BufferedReader r = new java.io.BufferedReader(new java.io.FileReader(f));
                String line = r.readLine();
                r.close();
        ...
        return 0;   // 视频挂件默认不额外旋转（SurfaceTexture 已处理朝向）
    }

    /** 挂件位置/尺寸（归一化 x y w h，左上原点），存 overlay_rect.txt。 */
    private float[] readOverlayRect() {
        float[] r = {0.6f, 0.02f, 0.38f, 0.38f};
        try {
            File f = new File(video_path + "overlay_rect.txt");
            if (f.exists()) {
                ...
                if (line != null) {
                    ...
                    if (p.length >= 4) {
                        r[0] = Float.parseFloat(p[0]); r[1] = Float.parseFloat(p[1]);
                        r[2] = Float.parseFloat(p[2]); r[3] = Float.parseFloat(p[3]);
                    }
                }
            }
        } catch (Exception ignored) {}
    ...
    private void process_camera2_init(Class hooked_class) {
        ...
        XposedHelpers.findAndHookMethod(hooked_class, "onOpened", CameraDevice.class, new XC_MethodHook() {
            ...
            protected void beforeHookedMethod(MethodHookParam param) throws Throwable {
                ...
                if (c2_local_player != null) {
                    c2_local_player.stop();
                    c2_local_player = null;
                }
                if (c2_player_1 != null) {
                    c2_player_1.stop();
                    c2_player_1.reset();
